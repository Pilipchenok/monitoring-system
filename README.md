...
На данном этапе развития проекта система построена на базе **Push-модели**: Агент активно инициирует соединение и отправляет метрики в Коллектор через REST API. 

**Ограничение:** Такая архитектура идеально работает внутри единого кластера (например, в Docker-сети) или если у Коллектора есть публичный статический IP-адрес. Однако, если Коллектор запущен на локальной машине, удаленные серверы не смогут отправить ему данные.

**Пути решения:**
1. **Переход на Pull-модель (в стиле Prometheus):** Инверсия контроля, при которой Коллектор сам опрашивает endpoint-ы Агентов (`GET /metrics`).
2. **Overlay-сети:** Объединение элементов в единую защищенную виртуальную сеть (например, с помощью Tailscale) без изменения исходного кода проекта.
3. **Туннелирование** Использование Ngrok для временного получения "белого" IP для Коллектора.
