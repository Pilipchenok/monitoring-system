...
### Масштабирование: Запуск с несколькими агентами (Multi-node setup)

Система изначально спроектирована для одновременного сбора метрик с множества серверов. Для настройки распределенного мониторинга выполните следующие шаги:

**1. Развертывание Коллектора (Серверная часть)**
Коллектор должен быть доступен для входящих HTTP-подключений от агентов. Запустите его одним из способов:
* На выделенном сервере с **публичным (белым) IP-адресом**.
* На локальной машине с использованием **туннелирования** (например, Ngrok или Cloudflare Tunnels), чтобы безопасно пробросить локальный порт 8080 в интернет.

**2. Настройка Агентов (Клиентская часть)**
На каждом целевом сервере, который необходимо мониторить, запускается свой независимый экземпляр Агента.
* Скопируйте исполняемый файл агента и файл `configs/agent_config.yaml` на целевой сервер.
* В файле `agent_config.yaml` **обязательно задайте уникальный `hostname`** (например, `ubuntu-db-1`, `macbook-worker`) и укажите актуальный `collector_url`.
* Запустите агента на сервере (например, через `systemd` или вручную: `./agent`).

Как только агент начнет отправлять данные, новый сервер автоматически появится в выпадающем списке на Live-дашборде Коллектора без необходимости перезагрузки.
